...
set CERT_FOLDER=%TEMP%\quic-cert
...
echo Generating self-signed certificate for CN=%CERT_NAME%...
powershell -Command ^
    "$cert = New-SelfSignedCertificate -DnsName '%CERT_NAME%' -CertStoreLocation 'Cert:\LocalMachine\My' -KeyExportPolicy Exportable -KeySpec Signature -HashAlgorithm SHA256 -FriendlyName 'MsQuic-Dev-Cert';" ^
    ...
    "Import-Certificate -FilePath '%CER_FILE%' -CertStoreLocation 'Cert:\LocalMachine\Root' | Out-Null;"
